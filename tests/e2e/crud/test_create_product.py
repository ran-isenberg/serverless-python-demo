...
def test_handler_200_ok(api_gw_url_slash_product: str, product_id: str, id_token: str):
...
def test_idempotency_does_not_affect_different_ids(api_gw_url_slash_product: str, id_token: str):
    # GIVEN a URL and product ID for creating a product
    # AND a valid request body for creating a product
    body = generate_create_product_request_body()
...
def test_handler_bad_request_invalid_body(api_gw_url_slash_product: str, product_id: str, id_token: str):
    # GIVEN a URL and product ID for creating a product
    ...
    response = requests.put(url=url_with_product_id, data=body_str, timeout=10, headers=get_auth_header(id_token))

    # THEN the response should indicate a bad request (HTTP 400)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_handler_bad_request_invalid_token(api_gw_url_slash_product: str):
    # GIVEN a URL and product ID for creating a product
    # AND an invalid request body missing valid id token in auth headers
    body_str = json.dumps({'price': 5})
    product_id = generate_product_id()
    url_with_product_id = f'{api_gw_url_slash_product}/{product_id}'

    # WHEN making a PUT request to create a product with the invalid body
    response = requests.put(url=url_with_product_id, data=body_str, timeout=10, headers=get_auth_header('aaaa'))
